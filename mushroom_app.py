        ...
        try:
            if status == 'colonizing' and b['inoculation_date']:
                ...
                if b['pinning_started_at']:
                    pin_date = date.fromisoformat(b['pinning_started_at'][:10])
                    elapsed  = (today - pin_date).days
                    rem_lo   = max(0, har_lo - elapsed)
                    rem_hi   = max(0, har_hi - elapsed)
                    proj_lo  = today + timedelta(days=rem_lo)
                    proj_hi  = today + timedelta(days=rem_hi)
                    proj_mid = today + timedelta(days=round((rem_lo + rem_hi) / 2))
                    note = 'Fruiting — est. remaining days to harvest'
                else:
                    # No pin date recorded — batch is actively fruiting, use half window
...
def species_edit(sp_id):
    ...
    if request.method == 'POST':
        ...
        conn.execute("""UPDATE species_db SET
            ...
            difficulty=?,
            fruiting_temp_lo_f=?, fruiting_temp_hi_f=?,
            colonization_temp_lo_f=?, colonization_temp_hi_f=?,
            ...
            (f.get('common_name', '').strip() or sp['common_name'],
             ...
             float(f['fruiting_temp_lo_f'])      if f.get('fruiting_temp_lo_f')      else None,
             float(f['fruiting_temp_hi_f'])      if f.get('fruiting_temp_hi_f')      else None,
             float(f['colonization_temp_lo_f'])  if f.get('colonization_temp_lo_f')  else None,
             float(f['colonization_temp_hi_f'])  if f.get('colonization_temp_hi_f')  else None,
